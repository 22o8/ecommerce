          ...
          <div class="flex items-center justify-between gap-3">
            <div>
              ...
              <div class="mt-1 text-sm text-[rgb(var(--muted))] rtl-text">
                اختر التصنيف المناسب ليتم عرض المنتجات الخاصة به مباشرة.
              </div>
            </div>
          </div>
            ...
            >
              ...
              <div class="p-4 text-center">
                ...
                <div class="mt-1 text-xs text-[rgb(var(--muted))] rtl-text">
                  {{ child.descriptionAr || 'عرض المنتجات المناسبة لهذا التصنيف' }}
                </div>
              </div>
            </NuxtLink>
          </div>
...
await useAsyncData(
  () => `category-root:${categoryKey.value}`,
  async () => {
...
const categoryItem = computed(
  () => (categories.value || []).find((c: any) => String(c.key || '').toLowerCase() === categoryKey.value) || null
)

const categoryLabel = computed(() => categoryItem.value?.nameAr || categoryKey.value)
const categoryDescription = computed(
  () => categoryItem.value?.descriptionAr || 'اختر القسم المناسب لتظهر لك المنتجات الخاصة بهذا التصنيف.'
)
...
async function loadChildren() {
  ...
  try {
    ...
    childSections.value = categoryItem.value?.hasDetailSections && parentId
      ? await fetchProblemChildren(parentId, 'regular')
      : []
