...
async function submit() {
  ...
  try {
    ...
    if (res?.welcomeOffer && process.client) {
      sessionStorage.setItem('drsb_pending_welcome_offer', JSON.stringify(res.welcomeOffer))
      window.dispatchEvent(new CustomEvent('drsb:welcome-offer'))
    }
    await router.replace('/account')
    if (process.client) {
      window.dispatchEvent(new CustomEvent('drsb:notifications-refresh'))
    }
  } catch (e: any) {
