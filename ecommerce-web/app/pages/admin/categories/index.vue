...
const form = reactive({ key: '', nameAr: '', nameEn: '', descriptionAr: '', descriptionEn: '', imageUrl: '', sortOrder: 0, isActive: true })

function resetForm() {
  editingId.value = ''
  Object.assign(form, { key: '', nameAr: '', nameEn: '', descriptionAr: '', descriptionEn: '', imageUrl: '', sortOrder: 0, isActive: true })
}

async function load() {
  loading.value = true
  try {
    items.value = await $fetch('/api/bff/admin/categories', { query: { _ts: Date.now() } }) as any[]
  } catch {
    items.value = []
  } finally {
    loading.value = false
