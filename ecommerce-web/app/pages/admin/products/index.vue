  ...
  <div class="space-y-4">
    ...
    <div class="admin-box">
      ...
      <div class="mt-4 grid gap-2 md:grid-cols-4">
        <input v-model="q" class="admin-input" placeholder="Search title or slug..." @keydown.enter="fetchList(1)" />
      ...
      <div v-else class="admin-table">
        ...
        <div v-for="p in pagedItems" :key="p.id" class="admin-tr">
          ...
          <div class="flex justify-end gap-2">
            ...
            <button class="admin-btn-danger" type="button" @click="removeOne(p)" :disabled="pending">Delete</button>
          </div>
        </div>

        <!-- Pagination (client-side بسيط حتى لو API ترجع Array) -->
        <div class="p-4 flex items-center justify-between">
          <div class="text-sm admin-muted">Page {{ page }} / {{ totalPages }}</div>
          <div class="flex gap-2">
            <button class="admin-pill" type="button" :disabled="page<=1" @click="go(page-1)">Prev</button>
            <button class="admin-pill" type="button" :disabled="page>=totalPages" @click="go(page+1)">Next</button>
          </div>
        </div>
      </div>
    </div>
