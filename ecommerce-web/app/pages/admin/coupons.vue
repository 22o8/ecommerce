<template>
  <div class="grid gap-6">
    <div>
      <h1 class="text-3xl font-black rtl-text text-[rgb(var(--text))]">{{ t('admin.couponsLabel') }}</h1>
      <p class="mt-1 text-sm text-muted rtl-text">إدارة الكوبونات بشكل واضح ودقيق مع شرح مختصر لكل قيمة.</p>
    </div>
        ...
        <div class="grid gap-4">
          ...
          <div class="grid gap-4 md:grid-cols-2">
            <div class="rounded-2xl border border-app bg-surface-2 p-4">
              <label class="mb-2 block text-sm font-bold rtl-text text-[rgb(var(--text))]">بداية التفعيل</label>
              <input
              ...
              />
              <p class="mt-2 text-xs text-muted rtl-text">اختياري. إذا تركته فارغًا يبدأ الكوبون مباشرة.</p>
            </div>

            <div class="rounded-2xl border border-app bg-surface-2 p-4">
              <label class="mb-2 block text-sm font-bold rtl-text text-[rgb(var(--text))]">نهاية التفعيل</label>
              <input
                v-model="form.endsAtUtc"
                type="datetime-local"
                class="w-full rounded-2xl border border-app bg-surface px-4 py-3 text-sm text-[rgb(var(--text))] outline-none transition focus:ring-0"
              />
              <p class="mt-2 text-xs text-muted rtl-text">اختياري. بعد هذا التاريخ يصبح الكوبون غير صالح تلقائيًا.</p>
            </div>
          </div>

          <label class="flex items-center gap-2 rounded-2xl border border-app bg-surface-2 px-4 py-3 text-sm text-[rgb(var(--text))]">
          ...
          <div class="flex flex-wrap gap-3">
            <UiButton @click="save">{{ editingId ? 'حفظ التعديلات' : 'إنشاء الكوبون' }}</UiButton>
      ...
      <div class="rounded-[28px] border border-app bg-surface p-5 shadow-sm">
        <div class="mb-4 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <div class="mt-1 text-sm text-muted rtl-text">{{ filteredItems.length }} من أصل {{ items.length }} كوبون</div>
          </div>
          <input
            v-model="search"
        ...
        <div v-if="!filteredItems.length" class="rounded-3xl border border-app bg-surface-2 p-8 text-center">
          <div class="text-lg font-bold rtl-text text-[rgb(var(--text))]">لا توجد كوبونات مطابقة</div>
          <div class="mt-2 text-sm text-muted rtl-text">جرّب تغيير البحث أو أنشئ كوبون جديد من النموذج.</div>
        </div>

        <div v-else class="grid gap-3">
          <div v-for="item in filteredItems" :key="item.id" class="rounded-3xl border border-app bg-surface-2 p-4 transition hover:shadow-sm">
            <div class="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
              <div class="min-w-0 flex-1">
                <div class="flex flex-wrap items-center gap-2">
                  <div class="font-black keep-ltr text-lg text-[rgb(var(--text))]">{{ item.code }}</div>
                  <span class="rounded-full px-3 py-1 text-xs font-bold"
                    :class="item.isActive ? 'bg-emerald-500/15 text-emerald-600 dark:text-emerald-300' : 'bg-red-500/15 text-red-600 dark:text-red-300'">
                    {{ item.isActive ? 'مفعّل' : 'متوقف' }}
                  </span>
                </div>

                <div class="mt-1 rtl-text text-base font-bold text-[rgb(var(--text))]">{{ item.title }}</div>
                ...
                <div class="mt-3 grid gap-2 sm:grid-cols-2 xl:grid-cols-3">
                  <div class="rounded-2xl border border-app bg-surface px-3 py-2 text-sm rtl-text">
                    ...
                    <div class="mt-1 font-bold text-[rgb(var(--text))]">{{ item.discountPercent }}%</div>
                  </div>
                  <div class="rounded-2xl border border-app bg-surface px-3 py-2 text-sm rtl-text">
                    <div class="text-muted">أقصى عدد للاستخدام</div>
                    <div class="mt-1 font-bold text-[rgb(var(--text))]">{{ item.maxUses ?? 'غير محدود' }}</div>
                  </div>
                  <div class="rounded-2xl border border-app bg-surface px-3 py-2 text-sm rtl-text">
                    <div class="text-muted">عدد الاستخدامات</div>
                    <div class="mt-1 font-bold text-[rgb(var(--text))]">{{ item.usedCount ?? 0 }}</div>
                  </div>
                </div>
                ...
                <div class="mt-3 grid gap-2 sm:grid-cols-2 text-sm">
                  <div class="rounded-2xl border border-app bg-surface px-3 py-2 rtl-text">
                    ...
                    <div class="mt-1 font-semibold text-[rgb(var(--text))]">{{ item.startsAtUtc ? fmtDate(item.startsAtUtc) : 'مباشر' }}</div>
                  </div>
                  <div class="rounded-2xl border border-app bg-surface px-3 py-2 rtl-text">
                    <div class="text-muted">نهاية التفعيل</div>
                    <div class="mt-1 font-semibold text-[rgb(var(--text))]">{{ item.endsAtUtc ? fmtDate(item.endsAtUtc) : 'بدون نهاية' }}</div>
                  </div>
                </div>
              </div>
...
function editItem(item:any){ editingId.value=item.id; Object.assign(form,{ code:item.code, title:item.title, discountPercent:item.discountPercent, fixedDiscountIqd:0, minimumOrderIqd:0, maxUses:item.maxUses||0, isActive:item.isActive, startsAtUtc:fromIso(item.startsAtUtc), endsAtUtc:fromIso(item.endsAtUtc) }) }
async function removeItem(id:string){ await api.del(`/admin/coupons/${id}`); await load(); if(editingId.value===id) resetForm() }
function fmtDate(v?:string){ return v ? new Date(v).toLocaleString('ar-IQ') : '' }
onMounted(load)
</script>
