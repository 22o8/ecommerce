...
<template>
  <div class="min-h-screen bg-ambient">
    <header class="sticky top-0 z-50">
      <div class="mx-auto max-w-6xl px-4 pt-4">
        <div class="card px-4 py-3 flex items-center justify-between">
          ...
          <nav class="hidden md:flex items-center gap-1 text-sm">
            ...
            <NuxtLink
              to="/services"
              class="px-3 py-2 rounded-xl hover:bg-[rgba(var(--text),0.06)] transition"
              :class="isActive('/services') ? 'nav-active' : ''"
            >
              {{ t('services') }}
            </NuxtLink>
            ...
            <NuxtLink
              ...
              to="/service-requests"
              class="px-3 py-2 rounded-xl hover:bg-[rgba(var(--text),0.06)] transition"
              :class="isActive('/service-requests') ? 'nav-active' : ''"
            >
              {{ t('serviceRequests') }}
            </NuxtLink>
        ...
        <div v-if="open" class="mt-2 card p-3 md:hidden">
          ...
          <NuxtLink to="/services" class="block px-3 py-2 rounded-xl hover:bg-[rgba(var(--text),0.06)]" @click="open = false">
            {{ t('services') }}
          </NuxtLink>
    ...
    <footer class="py-10 text-center text-sm muted">
      © {{ new Date().getFullYear() }} — Ecommerce Web
    </footer>
  </div>
</template>
